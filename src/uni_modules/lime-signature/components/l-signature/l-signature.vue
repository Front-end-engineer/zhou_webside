...
export default {
  ...
  async mounted() {
    ...
    this.offscreenWidth = this.canvasWidth =
      this.signature.canvas.get("width") * 2;
    this.offscreenHeight = this.canvasHeight =
      this.signature.canvas.get("height") * 2;
      ...
      return getRect(`#${this.canvasId}`, {
        ...
        if (res) {
          ...
          if (canvas) {
            ...
            canvas.width = width * 2 * pixelRatio;
            canvas.height = height * 2 * pixelRatio;
          } else {
            pixelRatio = 1;
